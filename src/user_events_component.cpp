...
UserEventsComponent::UserEventsComponent(const components::ComponentConfig& config,
  ...
  admin_channel.BindQueue(exchange_, removed_event_queue_, kUserRemovedRoutingKey, setup_deadline);
}

void UserEventsComponent::NotifyUserIsDeleted(std::int32_t id) {
  std::string message = std::to_string(id);
  rabbit_client_->PublishReliable(exchange_, kUserRemovedRoutingKey, message, rabbitmq::MessageType::kTransient,
                                  engine::Deadline::FromDuration(std::chrono::milliseconds{200}));
}

void UserEventsComponent::NotifyNewUserIsCreated(std::int32_t id) {
  std::string message = std::to_string(id);
  rabbit_client_->PublishReliable(exchange_, kUserCreatedRoutingKey, message, rabbitmq::MessageType::kTransient,
                                  engine::Deadline::FromDuration(std::chrono::milliseconds{200}));
}
